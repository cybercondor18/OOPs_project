...
public:
    ...
    {
        fstream outFile;
        outFile.open("jobseekers.csv", ios::app);
        if (outFile.is_open())
        {
...
void job::saveToFile()
{
    ofstream outFile("jobs.csv", ios::app);
    if (outFile.is_open())
    {
...
{
    ofstream outFile("applications.csv", ios::app);
    if (outFile.is_open())
    {
...
{
    ofstream outFile("employers.csv", ios::app);
    if (outFile.is_open())
    {
...
void job::create_job(string jb_title, string description, vector<string> req, float sal, string location, string jb_type)
{
    ...
    // Assign input values to class members
    job_title = jb_title;
    ...
    // Add requirements to job object
    for (int i = 0; i < req.size(); i++)
    {
...
m:
    ...
    case 1:
    up_skl:
        cout << "Enter the skill to update : ";
        cin.ignore();
        getline(cin, skl);
        for (int i = 0; i < this->skills.size(); i++)
        {
